...
The core idea behind this project is to abstraction in such a way that both traditional and Lie-group-based problems fall under the exact same interface. Using this, a single estimator implementations can operate on a variety of state definitions, such as the usual vector space, and any Lie group. We allow the user to define their custom state, process model, and measurement model, after which they will have a variety of algorithms available to them, including:
...
By implementing a few classes, the user can model a large variety of problems. 
