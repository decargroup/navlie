...
def test_state_interp_se3_position(direction):
    x0 = SE3State(SE3.Exp([0., 0., 0., 1,2,3]), 0, direction=direction)
    x1 = SE3State(SE3.Exp([0., 0., 0., 2,3,4]), 1, direction=direction)
...
def test_state_interp_se3_attitude(direction):
    ...
    x1 = SE3State(SE3.Exp([0., 0., 0., 0, 0, 0]), 1, direction=direction)
    x = state_interp(0.5, [x0, x1])
    assert np.allclose(x.stamp, 0.5)
...
def test_state_interp_out_of_bounds():
    x0 = SE3State(SE3.Exp([0., 0., 0., 1,2,3]), 0)
    x1 = SE3State(SE3.Exp([0., 0., 0., 2,3,4]), 1)
