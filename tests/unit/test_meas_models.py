from pynav.lib.states import MatrixLieGroupState, SO3State, SE2State, SE3State, SE23State, CompositeState
...
def _left_right_jacobian_test(x: MatrixLieGroupState, model: MeasurementModel, atol=1e-6, rtol=1e-4):
    x.direction = "left"
...
def test_global_position_se2():
    ...
    _left_right_jacobian_test(x, model, atol=1e-5)
...
def test_magnetometer_so3():
    x = SO3State(SO3.Exp([0.3, 0.1, 0.2]), stamp=0.0, state_id=2)
    model = Magnetometer(np.identity(3))
    _left_right_jacobian_test(x, model)

def test_magnetometer_se3():
    x = SE3State(SE3.Exp([0, 1, 2, 4, 5, 6]), stamp=0.0, state_id=2)
    model = Magnetometer(np.identity(3))
    _left_right_jacobian_test(x, model)


def test_magnetometer_se23():
    x = SE23State(SE23.Exp([1, 2, 3, 4, 5, 6, 7, 8, 9]), stamp=0.0, state_id=2)
    model = Magnetometer(np.identity(3))
    _left_right_jacobian_test(x, model)
...
def test_landmark_relative_position_se3():
    x = SE3State(SE3.Exp([0, 1, 2, 4, 5, 6]), stamp=0.0, state_id=2)
    model = PointRelativePosition([1,2,3], np.identity(3))
    _left_right_jacobian_test(x, model)


def test_landmark_relative_position_se23():
    x = SE23State(SE23.Exp([1, 2, 3, 4, 5, 6, 7, 8, 9]), stamp=0.0, state_id=2)
    model = PointRelativePosition([1,2,3], np.identity(3))
    _left_right_jacobian_test(x, model)
